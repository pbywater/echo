    ...
    `INSERT INTO memories
     ...
     VALUES
        ((SELECT users.id FROM users WHERE username = $1 OR email = $1),
          $2, $3, $4, $5, $6)
    `,
    // when we pass in memory_asset_url, heading and tag, line 17 will be replaced with 15
    //  [login, newMemory.memoryText, mediaType, newMemory.memory_asset_url, newMemory.heading, newMemory.tag]
    // $4, $5, $6 are hardcoded below for testing purposes
    [login, newMemory.memory_text, mediaType, 'dummyURL', 'dummyHeading', 'dummyTag'],
    (err, res) => {
      if (err) { callback(err); }
      callback(null, res);
    });
