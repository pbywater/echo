...
if (navigator.onLine) {
  ...
  console.log('offline');
  const centralNodesByTag = JSON.parse(localStorage.getItem('centralNodesByTagSaved'));
  const processedData = JSON.parse(localStorage.getItem('processedDataSaved'));
  console.log('processedData is ', processedData);
  const nodeDataArray = JSON.parse(localStorage.getItem('nodeDataArraySaved'));
  // Add unique tags to tag list for user to select from
  Object.keys(centralNodesByTag).forEach((tag) => {
    tag = tag.replace(/\W/g, '');
    $('.tags').append(
